...
public class TicTacToeGame {
	...
	*/
	...
	private int nextCheck2;
	private int resetCheck;
	private CellValue[] transform;
    ...
    *  String representation of the game
    ...
    public void reset(){
    	...
    	nextCheck = 0;
    	orig = 0;
 		transformedBoard = new int[this.board.length];
  		for(int j = 0; j < this.board.length; j++){
  			transformedBoard[j] = j;
  		}
    ...
    public void next(){
    	...
    	if(nextCheck < 1){
	    	transform = new CellValue[this.board.length];
    	...
    	if(lines == columns){
    		if(nextCount > 6){
    		...
    		else if((nextCount < 3) || (nextCount > 3)){
    			Utils.rotate(lines,columns,transformedBoard);
    			for(int k = 0; k < board.length; k++){
    				transform[k] = board[transformedBoard[k]];
    			...
    			for(int m = 0; m < board.length; m++){
    				transform[m] = board[transformedBoard[m]];
    			}
    			nextCount += 1;
    		}
    	}
    		...
    		else if((nextCount < 1) || (nextCount > 1)){
    			Utils.horizontalFlip(lines,columns,transformedBoard);
    			for(int k = 0; k < board.length; k++){
    				transform[k] = board[transformedBoard[k]];
    			}
  	...
  	*/    
  	public boolean equalsWithSymmetry(TicTacToeGame other){
		...
		while(this.hasNext()){
			...
			check = 0;
			for(int i = 0; i < this.board.length; i++){
				if(this.board[i] != other.board[transformedBoard[i]]){
					check = 1;
					break;
				}
			}
